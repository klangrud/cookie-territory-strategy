...
import { importTroopRowSchema } from "@/lib/validators/import.validators";
import { importTroops } from "@/lib/actions/import.actions";
...
export function TroopImport() {
  ...
  function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (evt) => {
      ...
      json.forEach((row, i) => {
        const parsed = importTroopRowSchema.safeParse(row);
        if (!parsed.success) {
  ...
  async function handleImport() {
    setLoading(true);
    try {
      const res = await importTroops(rows);
      setResult(res);
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-4">
          ...
          <table className="text-xs">
            <thead>
              <tr className="border-b">
                ...
                <th className="px-2 py-1 text-left">serviceUnitArea</th>
              </tr>
            </thead>
            <tbody>
              <tr className="text-gray-500">
                <td className="px-2 py-1">1234</td>
                <td className="px-2 py-1">Troop 1234</td>
                <td className="px-2 py-1">Daisy troop</td>
                <td className="px-2 py-1">North</td>
              </tr>
            </tbody>
          </table>
        </div>
        <p className="mt-2 text-xs text-gray-500">
          Only troopNumber is required. Existing troops will be updated with any
          new values provided. Extra columns in your file will be ignored.
        </p>
      </div>
      ...
      {rows.length > 0 && (
        <div className="space-y-2">
          ...
          >
            {loading ? "Importing..." : `Import ${validCount} Troop${validCount !== 1 ? "s" : ""}`}
          </button>
        </div>
      )}
      ...
      {result && (
        <div className="rounded border p-3 text-sm">
          <p className="font-medium text-green-700">
            Created {result.created} of {result.totalRows} troops
          </p>
