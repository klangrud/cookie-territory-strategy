...
import { importBoothRowSchema } from "@/lib/validators/import.validators";
import { importBooths } from "@/lib/actions/import.actions";
...
export function BoothImport() {
  ...
  function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (evt) => {
      ...
      json.forEach((row, i) => {
        const parsed = importBoothRowSchema.safeParse(row);
        if (!parsed.success) {
  ...
  async function handleImport() {
    setLoading(true);
    try {
      const res = await importBooths(rows);
      setResult(res);
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-4">
          ...
          <table className="text-xs">
            <thead>
              <tr className="border-b">
                ...
                <th className="px-2 py-1 text-left">startTime</th>
                <th className="px-2 py-1 text-left">endTime</th>
                <th className="px-2 py-1 text-left">boothType</th>
              </tr>
            </thead>
            <tbody>
              <tr className="text-gray-500">
                <td className="px-2 py-1">1234</td>
                <td className="px-2 py-1">Walmart</td>
                <td className="px-2 py-1">Front entrance</td>
                <td className="px-2 py-1">456 Oak Ave</td>
                <td className="px-2 py-1">Springfield</td>
                <td className="px-2 py-1">IL</td>
                <td className="px-2 py-1">62701</td>
                <td className="px-2 py-1">2026-03-15</td>
                <td className="px-2 py-1">09:00</td>
                <td className="px-2 py-1">14:00</td>
                <td className="px-2 py-1">storefront</td>
              </tr>
            </tbody>
          </table>
        </div>
        <p className="mt-2 text-xs text-gray-500">
          Only description and boothType are optional. Extra columns in your file will be ignored.
        </p>
      </div>
      ...
      {rows.length > 0 && (
        <div className="space-y-2">
          ...
          >
            {loading ? "Importing..." : `Import ${validCount} Booth${validCount !== 1 ? "s" : ""}`}
          </button>
        </div>
      )}
      ...
      {result && (
        <div className="rounded border p-3 text-sm">
          <p className="font-medium text-green-700">
            Created {result.created} of {result.totalRows} booths
          </p>
